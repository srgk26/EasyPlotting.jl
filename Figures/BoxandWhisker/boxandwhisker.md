This is just a quick heads up on the format your input dataset should be, to produce the desired Box and Whisker plot figure:

  1. Your dataset must have minimum 2 columns labelled with column names. Column 1 is just a dummy column that does not contribute to the plot in any way. But there still has to be a complete column 1 with a column name because of the standardisation required for the code. All data from column 2 onwards will be used to plot, with their column names as the tick labels in the x-axis.
